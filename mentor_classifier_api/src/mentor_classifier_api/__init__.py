    ...
    dictConfig(
        {
            "version": 1,
            "formatters": {
                "default": {"()": "mentor_classifier_api.JSONFormatter"},
                "simple": {"format": "%(levelname)s %(message)s"},
                "verbose": {
                    "format": "[%(asctime)s] - %(name)s: %(levelname)s - %(message)s [in %(pathname)s:%(lineno)d]"
                },
                "json": {"()": "mentor_classifier_api.JSONFormatter"},
                "request_json": {"()": "mentor_classifier_api.RequestJSONFormatter"},
            },
            "filters": {"requests": {"()": "mentor_classifier_api.RequestFilter"}},
            "handlers": {
                "console": {
